---
title: "Model B, Cumberland County, NJ Data Processing Workflow"
output:
  pdf_document:
    toc: yes
    toc_depth: '6'
...
```{r setup, include=FALSE, warning=FALSE, messages=FALSE, echo=TRUE, tidy=TRUE, cache=TRUE}
# default to suppress all code chunks unless explicitly indicated
knitr::opts_chunk$set(include=TRUE, warning=FALSE, error=FALSE, tidy=TRUE,
...
```{r neighborhoods-basemaps, include=TRUE, echo=TRUE, warning=FALSE, error=FALSE, tidy=TRUE, results='hide', message=FALSE}
...
```{r load-incidents, include=TRUE, echo=TRUE, warning=FALSE, error=FALSE, tidy=TRUE, results='hide', message=FALSE}
# incidents data
...
```{r agg-referrals, include=TRUE, echo=TRUE, warning=FALSE, error=FALSE, tidy=TRUE, results='hide', message=FALSE}
...
all_generic_fishnets_urban_rural <- st_read(
  '~/PredictAlign/PredictAlignFishnets/cumberland_fishnets_urban_areas_with_landscan_NJ_PLANAR.shp'
)
names(all_generic_fishnets_urban_rural) <- c(
  'net_id', 'net_pop', 'intake_year', 'fn_width', 'include_area',
...
fn05mi <- all_generic_fishnets_urban_rural %>% 
  filter(fn_width=='0.5' & intake_year=='2017') %>%
...
fnlandscan <- all_generic_fishnets_urban_rural %>% 
  filter(fn_width=='landscan' & intake_year=='2019') %>%
  st_drop_geometry %>% dplyr::select(
    net_id, include_area, in_Bridgeton, in_Commercial_Township, 
    in_Millville, in_Bridgeton, in_Vineland)
...
```{r finalize, include=TRUE, echo=TRUE, warning=FALSE, error=FALSE, tidy=TRUE, message=FALSE}
...
st_write(all_final, '~/PredictAlign/PredictAlignFishnets/cumberland_fishnets_urban_areas_and_maltreatment_aggregations_with_landscan_modelB_NJ_PLANAR.shp', delete_layer=TRUE)

write.csv(all_centroids, '~/PredictAlign/PredictAlignFishnets/cumberland_fishnets_urban_areas_and_maltreatment_aggregations_with_landscan_modelB_NJ_PLANAR.csv')
```
